...
class InStockBloc
    extends Bloc<InStockEvent, InStockState> {
  ...
  @override
  Stream<InStockState> mapEventToState(
      InStockEvent event) async* {
  ...
  Stream<InStockState> _mapGetInStockList() async* {
    ...
    if (jsonResponse.length > 0) {
      final _inStockList = jsonResponse.map((inStock) {
        return InStock(
              stockType: inStock["stockType"],
              totalOfQty: inStock["totalOfQty"]
            );
      }).toList();
